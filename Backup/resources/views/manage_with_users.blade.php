<x-layout>
    ...
    <div class="text-white font-mono flex-wrap flex min-h-min sm:min-w-min mb-10 py-4 bg-neutral-900 lg:mx-48 lg:mx-20 sm:mx-10 mx-10 md:my-10 my-5 px-5 rounded-3xl shadow-new">
        ...
        <div class="mb-20 text-center w-full">
            <a href="/manage/statsandusers" class="transition-all hover:tracking-kindof tracking-widest bg-neutral-600 py-3 px-5 rounded-lg shadow-newest hover:bg-neutral-400"> Get User </a>
        </div>
    </div>
    <div class="text-white font-mono flex-wrap flex min-h-min sm:min-w-min mb-10 py-4 bg-neutral-900 lg:mx-48 lg:mx-20 sm:mx-10 mx-10 md:my-10 my-5 px-5 rounded-3xl shadow-new">
        <p class="mt-5 mb-8 pb-5 w-full text-center">Users </p>
        <div class="mb-20 text-center w-full">
            <div class="px-4 flex min-h-[30px] w-full font-mono text-white my-2 py-2 rounded-lg bg-neutral-800">
                <div class="w-2/12" >username</div>
                <div class="w-2/12" >role</div>
                <div class="w-2/12" >email</div>
                <div class="w-2/12" >firstname</div>
                <div class="w-2/12" >lastname</div>
                <div class="w-2/12" >delete</div>
            </div>
            @foreach($users as $user)
                <div class="px-4 flex w-full font-mono text-white my-2 rounded-lg bg-neutral-700 items-center">
                    <div class="w-2/12 py-3" >{{$user->username}}</div>
                    <div class="w-2/12 py-3" >{{$user->is_admin ? "Admin" : "User"}}</div>
                    <div class="w-2/12 py-3" >{{$user->email}}</div>
                    <div class="w-2/12 py-3" >{{$user->firstname}}</div>
                    <div class="w-2/12 py-3" >{{$user->lastname}}</div>
                    <div class="w-2/12 py-3" >
                        @if($user->is_admin === 0)
                        <form method="POST" action="/userdelete/{{$user->id}}" class="w-full flex items-center">
                            @csrf
                            @method('DELETE')
                            <button class="w-full transition-all text-center hover:tracking-kindof tracking-widest mt-4 bg-neutral-400 py-1 px-2 rounded-lg shadow-newest hover:bg-red-700 flex-none" >Delete</button>
                        </form>
                        @else
                        {{"This User Is Admin"}}
                        @endif
